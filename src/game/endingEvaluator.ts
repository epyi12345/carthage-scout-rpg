...
export function evaluateEnding(state: GameState): Ending {
  ...
  const survival = Math.max(0, Math.round((state.playerState.health + state.playerState.warmth + (100 - state.playerState.fatigue) + state.playerState.morale) / 4));
  const returnTiming = Math.max(0, 100 - Math.max(0, state.currentDay - 1) * 12 - state.actionCount * 2);
  const score = {
    survival,
    mapAccuracy,
